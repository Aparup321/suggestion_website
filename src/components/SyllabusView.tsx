    ...
    <section className="flex flex-col gap-12 w-full max-w-5xl mx-auto py-10 px-4 md:px-0">
      ...
      <div className="flex flex-wrap items-center gap-3 px-6 bg-black p-6 border-[4px] border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] mx-4">
        <button
          ...
          className={`px-6 py-3 border-2 font-black text-[11px] uppercase tracking-widest transition-all ${
            !selectedSubjectId
          ...
          >
            <div className="flex flex-col items-start gap-4 border-l-[12px] border-[#00ef8e] pl-8 mb-12">
              ...
              {selectedSubjectId && (subject.objective || subject.outcomes || subject.prerequisites) && (
                <div className="grid md:grid-cols-2 gap-8 w-full mt-8 p-8 bg-[#1a1a1a] border-[4px] border-black shadow-[12px_12px_0px_0px_#000]">
                  {subject.objective && (
                    ...
                    <div className="space-y-3">
                      ...
                      <ul className="space-y-3">
                        {subject.outcomes.map((co, i) => (
                          <li key={i} className="text-xs text-slate-400 font-bold leading-snug border-l-4 border-black pl-4 bg-black/20 py-2">
                            {co}
                          </li>
                        ))}
                      </ul>
            ...
            <div className="grid gap-8">
              {subject.units.map((unit) => (
                <div key={unit.id} className="space-y-8 relative px-4 md:px-0 bg-[#00ef8e]/5 border-2 border-black p-8 shadow-[8px_8px_0px_0px_#000]">
